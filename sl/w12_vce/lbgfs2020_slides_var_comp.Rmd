...
```{r aov-result, echo=TRUE, results='markup'}
...
* Maximize $L(\mu, \Sigma)$ with respect to $\Sigma$
...
* Estimates of unknown quantity $\Sigma$ based on posterior distribution of unknowns given knowns
* Using Bayes Theorem:

\begin{align}
f(\Sigma | y) & =       \frac{f(\Sigma, y)}{f(y)} \notag \\
             & =       \frac{f(y | \Sigma)f(\Sigma)}{f(y)} \notag \\
             & \propto  f(y | \Sigma)f(\Sigma) \notag
\end{align}
...
* Fixed Linear Model with $\Sigma = \left[\begin{array}{c} \sigma_s^2  \\  \sigma_e^2 \end{array} \right]$
...
* Estimates are computed as empirical means and standard deviation based on the samples, e.g for $\sigma_s^2$

$$\widehat{\sigma_s^2}_{Bayes} =   {1 \over N}\sum_{t=1}^N (\sigma_s^2)^{(t)}$$
